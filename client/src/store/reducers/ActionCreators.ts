...
// export const tonConnectVerification =
//   (data: { proof: TonProof; account: Account }) =>
//   async (dispatch: AppDispatch) => {
//     try {
//       dispatch(verifySlice.actions.verifyFetching());
//       const response = await axios.post(
//         `${API_URL}${api.verify.tonConnectVerification}`,
//         data,
//         auth()
//       );
//       dispatch(verifySlice.actions.verifySuccess(response.data));
//     } catch (e: any) {
//       dispatch(verifySlice.actions.verifyError(e.response.data.message));
//     }
//   };
...
export const checkProof =
  (proof: TonProof, account: Account) => async (dispatch: AppDispatch) => {
    try {
      ...
      // Обработка успешного ответа от сервера
      dispatch(verifySlice.actions.verifySuccess(response.data));
      return response.data.isValidProof as boolean;
    } catch (error: any) {
      // Обработка ошибок
      dispatch(
        verifySlice.actions.verifyError(
